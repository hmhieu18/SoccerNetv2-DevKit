...
EVENT_DICTIONARY_V2_LITE = {"Penalty":0,"Goal":1,"Substitution":2,"Shots on target":3,
                                "Shots off target":4,
                                "Indirect free-kick":5,"Direct free-kick":6,"Corner":7,"Yellow card":8
                                ,"Red card":9,"Yellow->red card":10}

INVERSE_EVENT_DICTIONARY_V2_LITE = {0:"Penalty",1:"Goal",2:"Substitution",3:"Shots on target",
                                4:"Shots off target",5:"Indirect free-kick",6:"Direct free-kick",7:"Corner",8:"Yellow card"
                                ,9:"Red card",10:"Yellow->red card"}
import json
import zipfile
...
def evaluate(SoccerNet_path, Predictions_path, prediction_file="results_spotting.json", split="test", version=2, framerate=None, metric="loose", list_games=None):
    ...
    for game in tqdm(list_games):
        ...
        if version==1:
            ...
            num_classes = 3
        if version ==3:
            label_files = "Labels-v2-lite.json"
            num_classes = 11
...
def label2vector(labels, num_classes=17, framerate=2, version=2):
    ...
    for annotation in labels["annotations"]:
        ...
        if version == 3:
            if event not in EVENT_DICTIONARY_V2_LITE:
                continue
            label = EVENT_DICTIONARY_V2_LITE[event]
        elif version == 2:
