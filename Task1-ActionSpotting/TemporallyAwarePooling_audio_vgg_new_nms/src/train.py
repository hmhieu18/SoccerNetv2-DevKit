...
def train(dataloader,
    ...
    with tqdm(dataloader) as t:
        for i, (feats, audio_feats, labels) in enumerate(t):
    ...
    if not os.path.exists(output_results) or overwrite:
        ...
        with tqdm(dataloader) as t:
            for i, (game_ID, feat_half1, feat_half2, audio_feat_half1, audio_feat_half2, label_half1, label_half2) in enumerate(t):
                ...
                def get_spot_from_NMS(Input, Input2, window=60, thresh=0.0):
                ...
                for half, timestamp in enumerate([timestamp_long_half_1, timestamp_long_half_2]):
                  #          for half, timestamp in enumerate([timestamp_long_half_1]):
                    ...
                    for l in range(dataloader.dataset.num_classes):
                        ...
                        spots = get_spot(
                            timestamp[:, l], timestamp[:, l:l+1], window=NMS_window*framerate, thresh=NMS_threshold)
                        for spot in spots:
